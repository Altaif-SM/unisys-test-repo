...
def send_signup_email_to_agent(from_email, to_mail, subject, message,user_id, password, agent_id):
    from_email = settings.EMAIL_HOST_USER
    to = [to_mail, from_email]
    logo_url = 'http://unisys.online/static/images/university_logo.png'
    html_content = render_to_string('agent_email_template.html',{ 'message': message,'user_id':user_id,'password':password,'logo_url':logo_url, 'to_mail':to_mail,'agent_id':agent_id})
